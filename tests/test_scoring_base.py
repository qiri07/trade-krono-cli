...
class TestCompositeScorerABC:
    ...
    def test_concrete_impl_scores(self):
        scorer = _DummyScorer()
        assert scorer.score({"ticker": "sh.600519"}) == 42.0

    def test_concrete_impl_describe(self):
        scorer = _DummyScorer()
        assert scorer.describe() == "CompositeScorer[dummy_scorer]"

    def test_error_in_score_impl_propagates(self):
        scorer = _BadScorer()
    ...
    def test_config_passed_through(self):
        ...
        class CaptureScorer(CompositeScorer):
            ...
            def _score_impl(self, merged, config=None):
...
class TestRiskBoostStrategyABC:
    ...
    def test_concrete_impl_boosts(self):
...
class TestRatingMapperABC:
    ...
    def test_map_sell_text(self):
        ...
        assert s1 == Signal.BUY and c1 == 90.0
        assert s2 == Signal.SELL and c2 == 20.0
        assert s3 == Signal.HOLD and c3 == 50.0
    ...
    def test_map_chinese_text(self):
        ...
        assert s == Signal.BUY and c == 85.0
        s, c = mapper.map_rating("建议持有观望")
        assert s == Signal.HOLD and c == 55.0
...
class TestScorerRegistry:
    ...
    def test_builtin_strategies_loaded(self):
    ...
    def test_get_builtin_linear(self):
    ...
    def test_reset_via_module_function(self):
