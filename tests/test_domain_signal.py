...
def test_compute_ev_up():
    prob_win, prob_loss, avg_win, avg_loss, ev, raev = _compute_ev(
        direction=Direction.UP, expected_return=5.0, p10=-2.0, p90=12.0, cost_bps=17.0,
    )
...
def test_compute_ev_down():
    prob_win, prob_loss, avg_win, avg_loss, ev, raev = _compute_ev(
        direction=Direction.DOWN, expected_return=-3.0, p10=-10.0, p90=2.0, cost_bps=17.0,
    )
    assert ev is not None
    assert ev < 0
...
def test_compute_ev_flat():
    _, _, _, _, ev, _ = _compute_ev(
        direction=Direction.FLAT, expected_return=0.5, p10=-1.0, p90=2.0, cost_bps=17.0,
    )
    assert ev is not None
...
def test_compute_ev_nan_return():
    ...
    result = _compute_ev(
        direction=Direction.UP, expected_return=float("nan"),
        p10=-1.0, p90=6.0, cost_bps=17.0,
    )
...
def test_frozen():
    sa = SignalAssessment(
        ticker="sh.600519", eval_date="2026-08-11",
        final_signal=DomainSignal.HOLD, final_confidence=50.0,
    )
...
def test_to_dict():
    sa = SignalAssessment(
        ...
        final_signal=DomainSignal.BUY, final_confidence=80.0,
        conflict=SignalConflict.NONE,
        expected_value=5.5, prob_win=0.72, risk_adjusted_ev=1.2,
...
def test_from_dict():
    data = {
        ...
        "conflict": "none", "expected_value": 5.5, "prob_win": 0.72,
    }
...
def test_from_dict_invalid_signal():
    data = {
        "ticker": "sh.600519", "eval_date": "2026-08-11",
        "final_signal": "UNKNOWN", "final_confidence": 80.0,
    }
...
def test_empty():
    ...
    sa = SignalAssessment(
        ticker="sh.600519", eval_date="2026-08-11",
        final_signal=DomainSignal.HOLD, final_confidence=0.0,
    )
    assert sa.final_signal == DomainSignal.HOLD
