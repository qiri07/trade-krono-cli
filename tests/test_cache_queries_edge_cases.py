...
def cache_with_multi_kline(tmp_path: Path) -> tuple[Cache, CacheQueries]:
    ...
    df_a = pd.DataFrame({
        ...
        "open": [1700.0, 1710.0], "high": [1720.0, 1730.0],
        "low": [1690.0, 1700.0], "close": [1710.0, 1720.0],
        "volume": [1_000_000.0, 1_100_000.0],
    })
    df_b = pd.DataFrame({
        "timestamps": pd.to_datetime(["2026-08-01", "2026-08-03"]),
        "open": [50.0, 52.0], "high": [53.0, 54.0],
        "low": [49.0, 51.0], "close": [51.0, 53.0],
...
class TestCacheQueriesExportDailyPvH5:
    def test_export_with_h5(self, cache_with_multi_kline: tuple[Cache, CacheQueries], tmp_path: Path) -> None:
    ...
    def test_export_with_debug_insts(self, cache_with_multi_kline: tuple[Cache, CacheQueries], tmp_path: Path) -> None:
        _, qc = cache_with_multi_kline
        parquet_path = str(tmp_path / "out.parquet")
    ...
    def test_export_debug_h5_skipped_when_no_env(self, cache_with_multi_kline: tuple[Cache, CacheQueries], tmp_path: Path) -> None:
...
class TestCacheQueriesClearAllIdempotent:
    ...
    def test_clear_all_then_stats_zero(self, cache_with_multi_kline: tuple[Cache, CacheQueries]) -> None:
        _, qc = cache_with_multi_kline
        qc.clear_all()
        stats = qc.stats()
...
class TestCacheQueriesExportDailyPvDedup:
    def test_export_same_ticker_different_ranges(self, tmp_path: Path) -> None:
        ...
        df1 = pd.DataFrame({
            ...
            "low": [99.0, 101.0], "close": [101.0, 103.0],
            "volume": [1e6, 1.1e6],
        })
        df2 = pd.DataFrame({
            "timestamps": pd.to_datetime(["2026-08-03", "2026-08-04"]),
            "open": [104.0, 106.0], "high": [107.0, 108.0],
            "low": [103.0, 105.0], "close": [105.0, 107.0],
            "volume": [1.2e6, 1.3e6],
        })
