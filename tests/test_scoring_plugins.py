...
class TestLinearScorer:
    ...
    def test_high_confidence_high_change(self):
        """高置信 + 高预期涨幅 → 高分。"""
        s = LinearScorer()
        merged = _make_merged(
            ta_confidence=95.0,
    ...
    def test_low_confidence_down_direction(self):
        """低置信 + 下跌方向 → 低分。"""
        s = LinearScorer()
        merged = _make_merged(
            ta_confidence=30.0,
...
class TestMultiplicativeScorer:
    ...
    def test_linear_equivalent_at_zero_risk(self):
...
class TestRankBasedScorer:
    ...
    def test_fallback_to_linear_when_no_rank(self):
...
class TestDiminishingBoostBooster:
    ...
    def test_multiple_flags_diminished(self):
        """多标记叠加时边际递减。"""
        b = DiminishingBoostBooster()
...
class TestScoringStrategyConfig:
    ...
    def test_valid_rank_based(self):
