...
class TestRiskScore:
    def test_to_dict(self) -> None:
        s = RiskScore(ticker="sh.600519", date="2026-08-11",
                      volatility_score=30.0, drawdown_score=20.0,
                      liquidity_score=10.0, concentration_score=15.0,
                      market_regime_score=25.0, total_risk=25.0)
        d = s.to_dict()
    ...
    def test_print_report_contains_all_fields(self) -> None:
        s = RiskScore(ticker="sh.600519", date="2026-08-11",
                      volatility_score=30.0, drawdown_score=20.0,
                      liquidity_score=10.0, concentration_score=15.0,
                      market_regime_score=25.0, total_risk=22.0)
...
class TestRiskMetrics:
    def test_to_dict(self) -> None:
        m = RiskMetrics(
            ...
            var_95=-2.5, cvar_95=-3.1, beta=1.2,
            annualized_vol=0.25, max_drawdown=-0.18,
            volatility_score=30.0, drawdown_score=20.0,
            liquidity_score=10.0, gap_risk_score=15.0,
            event_risk_score=20.0, valuation_risk_score=25.0,
            concentration_score=12.0, market_regime_score=18.0,
            total_risk=20.0, return_adjustment=-0.05,
        )
        d = m.to_dict()
        assert d["var_95"] == -2.5
        assert d["beta"] == 1.2
    ...
    def test_print_report_with_values(self) -> None:
        m = RiskMetrics(
            ticker="sh.600519", date="2026-08-11",
            var_95=-2.34, cvar_95=-3.12, beta=1.15,
            annualized_vol=0.325, max_drawdown=-0.183,
            volatility_score=35.0, drawdown_score=28.0,
            liquidity_score=12.0, gap_risk_score=25.0,
            event_risk_score=42.0, valuation_risk_score=30.0,
            concentration_score=18.0, market_regime_score=28.0,
            total_risk=25.0, return_adjustment=-0.062,
        )
...
class TestRiskEngine:
    ...
    def test_custom_config(self) -> None:
        cfg = RiskConfig(
            weights=RiskWeights(
                volatility=0.15, drawdown=0.12, liquidity=0.10,
                concentration=0.08, market_regime=0.10,
                gap_risk=0.10, event_risk=0.10, valuation_risk=0.15,
            ),
        )
    ...
    def test_assess_minimal_data(self) -> None:
        ...
        df = type("DF", (), {
            ...
            "__getitem__": lambda self, key: type("S", (), {
                "iloc": lambda i: [0.0] * 5 if key == "close" else [f"2026-08-{i+1:02d}", 100.0, 101.0, 99.0, 100.0][[*"timestamps","open","high","low","close"].index(key)]
            })(),
        })()
