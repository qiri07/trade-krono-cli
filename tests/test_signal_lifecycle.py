...
class TestDetermineNextState:
    ...
    def test_hold_invalidates_buy(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.ACTIVE, 80.0, "HOLD", 60.0
        )
        assert state == SignalLifecycleState.INVALIDATED

    def test_sell_invalidates_any_active(self):
        for s in (SignalLifecycleState.ACTIVE, SignalLifecycleState.UPDATED,
                  SignalLifecycleState.WEAKENED, SignalLifecycleState.CREATED):
            state, _ = _determine_next_state(s, 70.0, "SELL", 50.0)
            assert state == SignalLifecycleState.INVALIDATED

    def test_invalidated_stays_invalidated_on_hold(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.INVALIDATED, 30.0, "HOLD", 55.0
        )
        assert state == SignalLifecycleState.INVALIDATED
        assert "终态" in reason
    ...
    def test_closed_stays_closed_on_sell(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.CLOSED, 20.0, "SELL", 90.0
        )
        assert state == SignalLifecycleState.CLOSED

    def test_reactivated_after_invalidated_with_strong_buy(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.INVALIDATED, 30.0, "BUY", 80.0
        )
        assert state == SignalLifecycleState.CREATED
        assert "重建" in reason
    ...
    def test_reactivated_after_closed_with_strong_buy(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.CLOSED, 10.0, "BUY", 85.0
        )
        assert state == SignalLifecycleState.CREATED

    def test_weakened_continues_weakened_when_low(self):
        state, reason = _determine_next_state(
            SignalLifecycleState.WEAKENED, 45.0, "BUY", 40.0
        )
        assert state == SignalLifecycleState.WEAKENED
...
class TestSignalLifecycle:
    ...
    def test_create_first_signal(self, lifecycle):
        record = lifecycle.update(
            ticker="sh.600519", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            job_id="job-001", run_id="run-001", thesis="基本面强劲",
        )
    ...
    def test_update_high_conf_promotes_to_updated(self, lifecycle):
        lifecycle.update(
            ticker="sh.600519", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            job_id="job-001", thesis="建仓",
        )
        record = lifecycle.update(
            ticker="sh.600519", date="2026-08-05",
            signal="BUY", confidence=82.0, composite_score=78.0,
            job_id="job-002", thesis="信心增强",
        )
    ...
    def test_hold_invalidates_buy(self, lifecycle):
        ...
        record = lifecycle.update(
            ticker="sh.600519", date="2026-08-10",
            signal="HOLD", confidence=61.0, composite_score=55.0,
            job_id="job-003", thesis="观望",
        )
    ...
    def test_sell_invalidates_buy(self, lifecycle):
        ...
        record = lifecycle.update(
            ticker="sh.600519", date="2026-08-15",
            signal="SELL", confidence=90.0, composite_score=92.0,
            job_id="job-004", thesis="获利了结",
        )
        assert record.lifecycle_state == SignalLifecycleState.INVALIDATED
    ...
    def test_low_confidence_becomes_weakened(self, lifecycle):
        ...
        record = lifecycle.update(
            ticker="sh.600519", date="2026-08-08",
            signal="BUY", confidence=45.0, composite_score=40.0,
            job_id="job-002", thesis="信心下滑",
        )
    ...
    def test_get_history_returns_all_records(self, lifecycle):
        for i, (date, sig, conf) in enumerate([
            ...
            lifecycle.update(
                ticker="sh.600519", date=date,
                signal=sig, confidence=conf, composite_score=conf - 5.0,
                job_id=f"job-00{i}", thesis=f"论{i}",
            )
    ...
    def test_describe_returns_readble_output(self, lifecycle):
        lifecycle.update(
            ticker="sh.600519", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            job_id="job-001", thesis="基本面强劲，估值合理",
        )
    ...
    def test_reactivated_after_invalidated(self, lifecycle):
        # Phase 1: create
        lifecycle.update(
            ticker="sz.000858", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            job_id="job-001", thesis="建仓",
        )
        # Phase 2: invalidated by sell
        lifecycle.update(
            ticker="sz.000858", date="2026-08-10",
            signal="SELL", confidence=90.0, composite_score=88.0,
            job_id="job-002", thesis="止损",
        )
        # Phase 3: reactivated with new strong BUY
        record = lifecycle.update(
            ticker="sz.000858", date="2026-08-20",
            signal="BUY", confidence=85.0, composite_score=82.0,
            job_id="job-003", thesis="重新建仓",
        )
        assert record.lifecycle_state == SignalLifecycleState.CREATED
...
class TestSignalRecord:
    ...
    def test_to_dict_and_from_dict_roundtrip(self):
        rec = build_signal_record(
            ...
            previous_state=None, transition_reason="首次",
            job_id="j1", run_id="r1", thesis="论点摘要",
        )
        d = rec.to_dict()
    ...
    def test_thesis_snapshot_truncated(self):
        ...
        rec = build_signal_record(
            ticker="sh.600519", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            lifecycle_state=SignalLifecycleState.CREATED, thesis=long_thesis,
        )
...
class TestResearchDbSignalHistory:
    ...
    def test_get_latest_signal_for_ticker_after_insert(self, lifecycle):
        lifecycle.update(
            ticker="sh.600519", date="2026-08-01",
            signal="BUY", confidence=80.0, composite_score=75.0,
            job_id="j1", run_id="r1", thesis="test",
        )
