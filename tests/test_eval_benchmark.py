...
class TestFetchBenchmarkKline:
    ...
    def test_fetch_raises_returns_none(self) -> None:
        with patch(
            "trade_krono_cli.eval_benchmark.fetch_kline", side_effect=RuntimeError("net err")
        ):
            assert fetch_benchmark_kline("sh.000300", "2026-01-01", "2026-01-02") is None
...
class TestComputeBenchmarkMetrics:
    ...
    def test_single_point_returns_none(self) -> None:
        with patch(
            "trade_krono_cli.eval_benchmark.fetch_benchmark_kline",
            return_value=[("2026-01-01", 100.0)],
        ):
            assert (
                compute_benchmark_metrics("sh.000300", "CSI300", "2026-01-01", "2026-01-01") is None
            )

    def test_empty_kline_returns_none(self) -> None:
        with patch("trade_krono_cli.eval_benchmark.fetch_benchmark_kline", return_value=[]):
            assert (
                compute_benchmark_metrics("sh.000300", "CSI300", "2026-01-01", "2026-01-02") is None
            )
    ...
    def test_zero_price_returns_none(self) -> None:
        with patch(
            "trade_krono_cli.eval_benchmark.fetch_benchmark_kline",
            return_value=[("2026-01-01", 0.0), ("2026-01-02", 100.0)],
        ):
            assert (
                compute_benchmark_metrics("sh.000300", "CSI300", "2026-01-01", "2026-01-02") is None
            )
        ...
        def side_effect(ticker, name, start, end):
            ...
            if name == "CSI300":
                return BenchmarkResult(
                    name=name, ticker=ticker, cumulative_return_pct=5.0, n_days=20
                )
            return None

        with patch(
            "trade_krono_cli.eval_benchmark.compute_benchmark_metrics", side_effect=side_effect
        ):
            result = compute_alpha(
                10.0,
                [type("ER", (), {"date": "2026-01-01"})()],
