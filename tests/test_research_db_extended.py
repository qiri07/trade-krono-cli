...
class TestSnapshots:
    def test_insert_and_get_snapshot(self, db):
        db.insert_data_snapshot(
            snapshot_id="snap-1", cut_date="2026-01-01",
            effective_cut="2025-12-31", sources=[{"name": "baostock"}],
    ...
    def test_insert_overwrites_snapshot(self, db):
        db.insert_data_snapshot(
            snapshot_id="snap-1", cut_date="2026-01-01",
            effective_cut="2025-12-31", sources=[{"name": "a"}],
        )
        db.insert_data_snapshot(
            snapshot_id="snap-1", cut_date="2026-02-01",
            effective_cut="2026-01-31", sources=[{"name": "b"}],
        )
        result = db.get_data_snapshot("snap-1")
...
class TestWalkforward:
    def test_insert_and_get_walkforward(self, db):
        db.insert_walkforward_run(
            run_id="wf-1", experiment_id="exp-1", ticker="600519",
            config={"pred_len": 30}, total_windows=10, valid_windows=8,
            win_rate=0.6, avg_return=0.02, sharpe_annual=1.5,
            n_records=100, elapsed_sec=5.0,
        )
    ...
    def test_get_walkforward_filtered(self, db):
        db.insert_walkforward_run(
            run_id="wf-1", experiment_id="exp-1", ticker="600519",
            config={}, total_windows=5, valid_windows=4,
            win_rate=0.8, avg_return=0.01, sharpe_annual=1.0,
            n_records=50, elapsed_sec=2.0,
        )
        db.insert_walkforward_run(
            run_id="wf-2", experiment_id="exp-2", ticker="000858",
            config={}, total_windows=3, valid_windows=2,
            win_rate=0.5, avg_return=0.005, sharpe_annual=0.5,
            n_records=30, elapsed_sec=1.0,
        )
    ...
    def test_get_walkforward_by_experiment(self, db):
        db.insert_walkforward_run(
            run_id="wf-1", experiment_id="exp-A", ticker="600519",
            config={}, total_windows=5, valid_windows=4,
            win_rate=0.8, avg_return=0.01, sharpe_annual=1.0,
            n_records=50, elapsed_sec=2.0,
        )
...
class TestExperiments:
    def test_insert_and_get_experiment(self, db):
        db.insert_experiment(
            experiment_id="exp-1", full_id="full-1",
    ...
    def test_list_experiments(self, db):
        db.insert_experiment(
            experiment_id="exp-1", full_id="full-1",
            experiment_type="ablation", hypothesis={}, passed=True,
        )
        db.insert_experiment(
            experiment_id="exp-2", full_id="full-2",
            experiment_type="comparison", hypothesis={}, passed=False,
        )
    ...
    def test_list_experiments_filtered_by_type(self, db):
        ...
        db.insert_experiment(
            experiment_id="exp-2", full_id="full-2",
            experiment_type="comparison", hypothesis={}, passed=True,
        )
    ...
    def test_list_experiments_only_passed(self, db):
        ...
        db.insert_experiment(
            experiment_id="exp-2", full_id="full-2",
            experiment_type="ablation", hypothesis={}, passed=False,
        )
    ...
    def test_insert_with_none_values(self, db):
        db.insert_experiment(
            experiment_id="exp-1", full_id="full-1",
            experiment_type="ablation", hypothesis={"a": 1},
            passed=None,
        )
        result = db.get_experiment("exp-1")
