...
class TestClassifyError:
    ...
    def test_retryable_explicit_classes(self):
    ...
    def test_non_retriable_explicit_classes(self):
    ...
    def test_runtime_error_with_network_msg(self):
    ...
    def test_runtime_error_with_data_not_found_msg(self):
        exc = RuntimeError("no data for sh.600519")
        cat, _ = classify_error(exc)
        assert cat == "non_retriable"

    def test_runtime_error_with_auth_msg(self):
        exc = RuntimeError("invalid api key 401 unauthorized")
        cat, _ = classify_error(exc)
        assert cat == "non_retriable"

    def test_runtime_error_with_rate_limit_msg(self):
        exc = RuntimeError("rate limit exceeded 429")
        cat, _ = classify_error(exc)
        assert cat == "retriable"

    def test_generic_exception_defaults_to_non_retriable(self):
    ...
    def test_5xx_status_in_message(self):
...
class TestSmartRetry:
    ...
    def test_retries_on_retriable_error_then_succeeds(self):
        policy = RetryPolicy(max_attempts=3, base_delay=0.01, jitter=False)
        call_count = 0
    ...
    def test_gives_up_after_max_attempts_for_retriable(self):
    ...
    def test_non_retriable_error_raises_immediately(self):
        ...
        def fn():
            nonlocal call_count
            call_count += 1
            raise ParameterError("invalid ticker")
    ...
    def test_rate_limit_with_retry_after(self):
        ...
        def fn():
            ...
            call_count += 1
            if call_count < 2:
                raise make_rate_limit_error("429", headers={"Retry-After": "0.05"})
            return "done"
    ...
    def test_rate_limit_without_retry_after_falls_back_to_exponential(self):
        ...
        def fn():
            ...
            call_count += 1
            if call_count < 3:
                raise make_rate_limit_error("429 no header")
            return "ok"
    ...
    def test_non_retryable_error_skipped(self):
        ...
        @smart_retry(policy)
        def fn():
            raise DataNotFoundError("stock delisted")

        with pytest.raises(DataNotFoundError):
            fn()

    def test_explicit_retryable_subclass_is_retried(self):
        """Any TradeKronoRetryableError subclass is retried."""
        policy = RetryPolicy(max_attempts=2, base_delay=0.01, jitter=False)

        @smart_retry(policy)
        def fn():
            raise Server5xxError("502 bad gateway", status_code=502)
...
class TestFailureStore:
    ...
    def test_record_updates_existing_entry(self, store):
    ...
    def test_list_fails_filters_by_module(self, store):
    ...
    def test_list_fails_filters_by_category(self, store):
        store.record("sh.600519", "2026-01-15", "ta", NetworkError("timeout"))
    ...
    def test_get_tickers_returns_unique_list(self, store):
    ...
    def test_clear_for_date(self, store):
        store.record("sh.600519", "2026-01-15", "ta", NetworkError("e1"))
...
class TestMakeRateLimitError:
    ...
    def test_with_x_ratelimit_reset(self):
...
class TestPipelineConfigRetry:
    ...
    def test_override_retry_fields(self):
...
class TestConfigValidatorRetry:
    ...
    def test_base_delay_too_large(self):
        clear_settings()
    ...
    def test_rate_limit_max_wait_zero(self):
        clear_settings()
    ...
    def test_rate_limit_max_wait_too_large(self):
        clear_settings()
