...
class TestHashUserPromptStructural:
    ...
    def test_different_ticker_different_hash(self):
        h1 = hash_user_prompt_structural(
            ticker="sh.600519", date="2026-08-11", analysts=[]
        )
        h2 = hash_user_prompt_structural(
            ticker="sz.000858", date="2026-08-11", analysts=[]
        )
        assert h1 != h2
...
class TestLLMRequest:
    ...
    def test_roundtrip(self):
        r = LLMRequest(
            source="ta", provider="deepseek", model="deepseek-chat",
            temperature=0.3, success=True, latency_sec=2.5,
        )
        d = r.to_dict()
...
class TestBuildTARequest:
    ...
    def test_failure(self):
        r = build_ta_llm_request(
            ...
            system_prompt="system", analysts=["market"],
            latency_sec=1.0, success=False,
