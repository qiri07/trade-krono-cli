...
class TestAbnormalityConfig:
    ...
    def test_validate_new_stock_min_days_too_low(self) -> None:
        cfg = AbnormalityConfig(new_stock_min_days=3)
        errors = cfg.validate()
        assert len(errors) == 1
        assert "new_stock_min_days" in errors[0]

    def test_validate_new_stock_min_days_boundary(self) -> None:
        # 最小允许值是 5
        cfg = AbnormalityConfig(new_stock_min_days=5)
        errors = cfg.validate()
        assert errors == []

    def test_validate_completeness_above_one(self) -> None:
        cfg = AbnormalityConfig(kline_min_completeness=1.5)
        errors = cfg.validate()
        assert len(errors) == 1
        assert "kline_min_completeness" in errors[0]

    def test_validate_completeness_zero(self) -> None:
        cfg = AbnormalityConfig(kline_min_completeness=0.0)
        errors = cfg.validate()
        assert len(errors) == 1
        assert "kline_min_completeness" in errors[0]

    def test_validate_completeness_negative(self) -> None:
        cfg = AbnormalityConfig(kline_min_completeness=-0.1)
        errors = cfg.validate()
        assert len(errors) == 1

    def test_validate_completeness_valid_boundary(self) -> None:
    ...
    def test_merge_completeness(self) -> None:
