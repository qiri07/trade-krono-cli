...
class TestCheckKlineCompleteness:
    ...
    def test_none_df_fails(self):
        passed, reason = check_kline_completeness(None, "sh.600519")
        assert passed is False

    def test_missing_timestamps_column(self):
        df = pd.DataFrame({"close": [1.0, 2.0]})
        passed, reason = check_kline_completeness(df, "sh.600519")
        assert passed is False
        assert "timestamps" in reason

    def test_with_gap_below_threshold(self):
        """5% 缺失率，低于 85% 阈值 → 不通过。"""
        df = self._make_df(n_rows=100, gap_at=50)
        passed, reason = check_kline_completeness(df, "sh.600519", min_completeness=0.95)
        # 有较大缺口，完整率低于 95%
        assert passed is False
...
class TestApplyRiskBoost:
    ...
    def test_single_suspended(self):
    ...
    def test_delisted_heavy(self):
        result = apply_abnormality_risk_boost(30.0, ["DELISTED"])
        assert result == 80.0

    def test_new_stock_light(self):
    ...
    def test_unknown_flag_safe(self):
        """未知标记不应崩溃。"""
...
class TestPrecheckStockStatus:
    def test_all_normal(self, monkeypatch):
        ...
        for ticker, flag in results.items():
