...
class TestScaledBoostBooster:
    ...
    def test_default_multiplier(self, booster: ScaledBoostBooster) -> None:
        """默认倍率为 1.0，行为与 fixed_boost 一致。"""
        result = booster.boost(base_risk=30.0, flags=["ST"], params={})
        assert result == pytest.approx(50.0)

    def test_mild_multiplier(self, booster: ScaledBoostBooster) -> None:
        """温和倍率 0.5 → 减半加分。"""
        result = booster.boost(base_risk=30.0, flags=["ST"], params={"multiplier": 0.5})
        # ST=20 * 0.5 = 10
        assert result == pytest.approx(40.0)

    def test_aggressive_multiplier(self, booster: ScaledBoostBooster) -> None:
        """激进倍率 2.0 → 加倍加分。"""
        result = booster.boost(base_risk=30.0, flags=["ST"], params={"multiplier": 2.0})
        # ST=20 * 2.0 = 40
        assert result == pytest.approx(70.0)

    def test_name(self, booster: ScaledBoostBooster) -> None:
...
class TestDiminishingBoostBooster:
    ...
    def test_single_flag(self, booster: DiminishingBoostBooster) -> None:
        """单个标记应正常加分。"""
        result = booster.boost(base_risk=30.0, flags=["ST"], params={})
        assert result == pytest.approx(50.0)

    def test_multiple_flags_diminishing(self, booster: DiminishingBoostBooster) -> None:
        """多个标记应平方根缩放，防止过度惩罚。"""
    ...
    def test_name(self, booster: DiminishingBoostBooster) -> None:
        assert booster.name == "diminishing_boost"

    def test_cap_at_100(self, booster: DiminishingBoostBooster) -> None:
        """加分后不应超过 100。"""
...
class TestRiskBoostStrategyRegistry:
    ...
    def test_fixed_boost_registered(self) -> None:
        """fixed_boost 策略应可获取。"""
    ...
    def test_scaled_boost_registered(self) -> None:
        """scaled_boost 策略应可获取。"""
        from trade_krono_cli.scoring.registry import get_risk_boost_registry
    ...
    def test_diminishing_boost_registered(self) -> None:
