...
class TestAnnualizedVar:
    ...
    def test_annualized_cvar(self) -> None:
        """年化 CVaR 计算正确。"""
        returns = _make_returns([-1.0, -2.0, -1.5, -0.5] * 50)
...
class TestBeta:
    ...
    def test_no_market_returns(self) -> None:
        """无市场数据 → 返回默认值 1.0。"""
...
class TestExpectedReturnAdjustment:
    def test_low_risk_positive_adj(self) -> None:
        """低风险 → 调整因子接近 0（轻微加成）。"""
        metrics = {
            "var_95": -0.5,
            "cvar_95": -0.7,
    ...
    def test_high_risk_negative_adj(self) -> None:
        """高风险 → 调整因子显著为负。"""
        metrics = {
            "var_95": -5.0,
            "cvar_95": -7.0,
    ...
    def test_all_none_neutral(self) -> None:
        """全 None → 中性调整（≈0）。"""
        adj = expected_return_adjustment({})
        # 全部 None → 每个维度归一化为 0.5，加权后 ≈ -0.05 ~ 0
        assert -0.15 < adj <= 0.0

    def test_adjust_expected_return(self) -> None:
...
class TestGapRisk:
    def test_stable_prices_low_gap_risk(self) -> None:
        """平稳价格 → 低缺口风险。"""
    ...
    def test_high_volatility_high_gap_risk(self) -> None:
        """高波动 → 高缺口风险。"""
        np.random.seed(42)
...
class TestEventRisk:
    def test_stable_event_risk(self) -> None:
        """稳定波动 → 中等事件风险。"""
        np.random.seed(0)
    ...
    def test_recent_vol_spike_high_event_risk(self) -> None:
        """近期波动率骤升 → 高事件风险。"""
        # 前 50 日平稳，后 30 日高波动
        vals = [100.0] * 50
...
class TestValuationRisk:
    def test_reasonable_valuation_low_risk(self) -> None:
        """合理估值 → 低估值风险。"""
        score = valuation_risk_score(pe_ttm=15.0, pb=1.5, market_cap_billion=500.0)
        assert score < 40

    def test_overvalued_high_risk(self) -> None:
        """高 PE → 高估值风险。"""
        score = valuation_risk_score(pe_ttm=150.0, pb=10.0)
        assert score > 60

    def test_loss_making_high_risk(self) -> None:
        """亏损股 → 高估值风险。"""
        score = valuation_risk_score(pe_ttm=-5.0, pb=1.0)
        assert score > 50

    def test_small_cap_extra_risk(self) -> None:
        """小市值 → 更高估值风险。"""
