...
def _make_mock_repo(tmp_path: Path, name: str, commit: str = "abc123def456") -> Path:
    ...
    (repo_dir / ".git" / "HEAD").write_text(f"ref: refs/heads/main\n")
...
class TestConfigIO:
    ...
    def test_save_and_load_roundtrip(self, tmp_path):
        ...
        original = {
            ...
            "repo_b": {"path": "ext/b", "branch": "develop", "url": "https://example.com/b", "commit": "aaa111"},
        }
...
class TestPin:
    def test_pin_creates_config(self, tmp_path):
        ...
        from trade_krono_cli.external import pin, save_config, load_config
    ...
    def test_pin_invalid_repo_raises(self, tmp_path):
        ...
        save_config({"other_repo": {"path": str(tmp_path / "other"), "branch": "main", "url": "", "commit": None}}, tmp_path)
...
class TestGetReproInfo:
    def test_returns_repo_info(self, tmp_path):
        ...
        save_config({
            "tradingagents": {"path": str(tmp_path / "ta"), "branch": "main", "url": "", "commit": None},
            "kronos":        {"path": str(tmp_path / "kr"), "branch": "main", "url": "", "commit": "abc123def456"},
        }, tmp_path)
...
class TestLockFile:
    ...
    def test_save_and_load_roundtrip(self, tmp_path):
        """保存再加载 lock 文件应保持一致。"""
        from trade_krono_cli.external import save_lock, load_lock

        data = {
            "generated_at": "2026-08-11T12:00:00",
            "repos": {
                "tradingagents": {"commit": "aaa111", "commit_short": "aaa111", "pinned_at": "2026-08-11T12:00:00", "branch": "main", "dirty": False},
                "kronos":        {"commit": "bbb222", "commit_short": "bbb222", "pinned_at": "2026-08-11T12:00:00", "branch": "main", "dirty": True},
            }
        }
    ...
    def test_get_locked_commit(self, tmp_path):
        ...
        from trade_krono_cli.external import save_lock, get_locked_commit
    ...
    def test_update_lock(self, tmp_path):
        ...
        from trade_krono_cli.external import update_lock, load_lock
    ...
    def test_status_detects_lock_mismatch(self, tmp_path):
        ...
        from trade_krono_cli.external import save_lock, ExternalRepo, _get_git_status
        ...
        save_lock({
            "generated_at": "2026-08-11T12:00:00",
            "repos": {
                "mismatch_repo": {"commit": "abc123def456789", "commit_short": "abc123def456", "branch": "main", "dirty": False}
            }
        }, tmp_path)
