...
class Settings:
    ...
    backend_url: str | None = field(default_factory=lambda: os.getenv("BACKEND_URL", None))
    ...
    provider_rank_cache_ttl_sec: int = field(
        default_factory=lambda: int(os.getenv("PROVIDER_RANK_CACHE_TTL_SEC", "600")),
    )
    """Provider 自适应排序缓存 TTL（秒），默认 600（10 分钟）。"""
    provider_bench_workers: int = field(
        default_factory=lambda: int(os.getenv("PROVIDER_BENCH_WORKERS", "3")),
    )
    """Provider benchmark 并发线程数，默认 3。"""
